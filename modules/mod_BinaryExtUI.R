BinaryExtUI <- function(id = "BinaryExt") {
  ...
  fldRow <- fluidRow(
    box(width = 12,
        ...
        wellPanel(
          withMathJax(includeMarkdown("documents/BinaryExternal.Rmd"))
        ),
        wellPanel(
          fluidRow(column(12,
                          HTML("<b>Confidence Levels of Making Go/NoGo Decision:</b>"))),
          fluidRow(column(3, numericInput(ns("gamma_1"), label = withMathJax("\\(\\gamma_1\\)"), value = 0.6)),
        ...
        wellPanel(
          ...
          fluidRow(column(12, HTML("<b>External Data Response Number:</b>"))),
          fluidRow(
            column(3,numericInput(ns("y_et"), label = withMathJax("\\(y_{et}\\)"), value = 6)),
            column(3,numericInput(ns("y_ec"), label = withMathJax("\\(y_{ec}\\)"), value = 6))
          ),
        ),
        tabsetPanel(
