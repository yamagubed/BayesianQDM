...
BinaryWithoutUI <- function(id = "BinaryWithout") {
  ns <- NS(id)
  fldRow <- fluidRow(
    box(width = 12,
        title = "Binary Endpoint: Without Controlled PoC Study",
        wellPanel(
          withMathJax(includeMarkdown("documents/BinaryWithout.Rmd"))
          ),
        wellPanel(
          fluidRow(column(12,
          ...
          fluidRow(column(12,
                          HTML("<b>Parameters for the Prior Distribution and Hypothetical Control: </b>"))),
          fluidRow(
            ...
            column(3, numericInput(ns("z"), label = withMathJax("$$z$$"), value = 0.5))
          ),
          fluidRow(column(12,
                          HTML("<b>Sample Size in PoC Study: </b>"))),
          fluidRow(column(3, numericInput(ns("n_t"), label = withMathJax("$$n_t$$"), value = 12))),
          fluidRow(
            column(12,
                   HTML("<b>Assumed Response Rate for Operating Characteristic</b>"))
          ),
          fluidRow(column(6,
        ...
        tabsetPanel(
          ...
          tabPanel("Posterior Probability as Decision Criteria",
                   ...
                   box(width = 12,
                       ...
                       wellPanel(
                         fluidRow(
                           column(12, verbatimTextOutput(ns("Probability_pos")))
                         ))
                       )),
          tabPanel("Predictive Probability as Decision Criteria",
                   ...
                   box(width = 12,
                       ...
                       wellPanel(
                         fluidRow(
                           column(12, verbatimTextOutput(ns("Probability_pred")))
                         ))))
        )
    ),
  )
